...
- **IP-to-Geo Enrichment:** The raw visitor IP is resolved to geographic metadata (country, city, region, lat/lon) via a 3-provider fallback chain of free, HTTPS-only services (`ipwho.is` → `ipapi.co` → `freeipapi.com` → `unknown`). Private or loopback IP addresses (e.g., `127.0.0.1`, `10.x.x.x`) are securely short-circuited without triggering any network calls. The timeout for these requests is controlled by the `GEO_TIMEOUT_MS` environment variable.
